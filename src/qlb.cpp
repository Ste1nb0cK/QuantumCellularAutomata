...
class QLB {
private:
  complex Psi[Q * L]; // Wave vectors are created as private attributes.
  complex Psi_new[Q * L];
...
complex QLB::Rho(int ix) { return Psi[ix] + Psi[ix + 1]; }
...
int main() {
  ...
  // Piece of code for printing matrices.
  /* for(int j=0; j<Q*L; j++){
   for(int i=0; i<Q*L; i++)
     std::cout<<M[i][j]<<" ";
     std::cout<<std::endl; }*/
